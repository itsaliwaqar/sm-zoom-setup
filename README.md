...
- Post-event attendance sync: a few minutes (configurable in Settings, default 10) after each event ends, pulls Zoom's attendee report, marks each registrant attended/no-show, and per their registration route's config tags GHL, tags Hyros, and/or forwards to an outbound webhook. Registrants (with attendance) can be exported as CSV per event.
- Series, Templates, and Zoom Events (webinars/meetings) can all be edited after creation; events can also be deleted (removes from Zoom - irreversible there - but keeps local registrant/attendance history). Scheduled jobs have a "run now" action to reconcile immediately instead of waiting for the next cron tick.
